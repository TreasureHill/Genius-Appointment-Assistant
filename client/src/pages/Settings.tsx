...
type Data = { providers: Providers; pacing: Pacing };

export function Settings() {
  const [data, setData] = useState<Data | null>(null);
  useEffect(() => { api.get<Data>("/api/settings").then(setData); }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <PacingCard initial={data.pacing} />
    </div>
  );
}
...
function ProvidersCard({ providers }: { providers: Providers }) {
  ...
  async function runTest(channel: "EMAIL" | "SMS") {
    ...
    try {
      await api.post("/api/settings/test", { channel, to });
      setMsg(`${channel} test sent.`);
    } catch (err) {
  ...
  const Row = ({ label, on, vars }: { label: string; on: boolean; vars: string[] }) => (
    <div className="flex items-start justify-between rounded-md border border-slate-200 p-3">
      <div>
        ...
        <div className="text-xs text-slate-500">Configured via env: <code>{vars.join(", ")}</code></div>
      </div>
    ...
    <div className="card space-y-4">
      <div>
        ...
        <p className="text-xs text-slate-500">
          Credentials live in <code>.env</code>. Change them there and restart the app.
        </p>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <input className="input max-w-sm" placeholder="you@example.com or +15551234567"
...
function PacingCard({ initial }: { initial: Pacing }) {
  ...
  return (
    <form className="card space-y-3" onSubmit={async (e) => {
      e.preventDefault();
      try {
        await api.post("/api/settings", { key: "pacing", value: s });
        setMsg("Saved.");
      } catch (err) {
